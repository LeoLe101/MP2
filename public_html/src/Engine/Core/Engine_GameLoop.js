...
gEngine.GameLoop = (function () {
    var kFPS = 180;          // Frames per second
    ...
    var _runLoop = function () {
        if (mIsLoopRunning) {
            ...
            while ((mLagTime >= kMPF) && mIsLoopRunning) {
                ...
                mLagTime -= kMPF;
                mUpdateCallsPerDraw += 1;
            }

            // Step D: now let's draw
            this.draw();    // Call MyGame.draw()
            
            gUpdateFrame(mElapsedTime, mUpdateCallsPerDraw, printLag);

            mUpdateCallsPerDraw = 0;
        }
    };
